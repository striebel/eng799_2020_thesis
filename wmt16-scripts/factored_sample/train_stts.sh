...
CUDA_VISIBLE_DEVICES=$device python $nematus/nematus/train.py  \
  --model 'model/stts/model_stts.npz'                          \
  --reload latest_checkpoint                                   \
  --dim_word 500                                               \
  --factors 3                                                  \
  --dim_per_factor 467 7 26                                    \
  --state_size 1024                                            \
  --decay_c 0.0                                                \
  --clip_c 1.0                                                 \
